...
if (isset($_GET['search'])) {
    $search_query = $_GET['search']; // Store the search query submitted by the user
}

// If the search form is submitted, fetch the filtered regular customers
if (!empty($search_query)) {
    $stmt = $conn->prepare("SELECT * FROM customer_tbl WHERE customerClass_id = 1 
                            AND (customer_id LIKE :search_query 
                            OR customer_firstname LIKE :search_query 
                            OR customer_lastname LIKE :search_query 
                            OR customer_contactNo LIKE :search_query 
                            OR customer_houseno LIKE :search_query 
                            OR customer_street LIKE :search_query 
                            OR customer_city LIKE :search_query 
                            OR date_created LIKE :search_query 
                            OR date_updated LIKE :search_query) 
